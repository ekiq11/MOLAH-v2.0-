...
void main() {
  patchSplashScreen();
  patchLoginScreen();
  print('Successfully patched UI for Splash and Login');
}
...
void patchSplashScreen() {
  ...
  final endMarker = '}\\n'; // Last bracket of the class
  
  // Custom regex to find the build method
  final buildIndex = content.indexOf('  @override\n  Widget build(BuildContext context) {');
  if (buildIndex == -1) {
    print('Could not find build method in splashscreen.dart');
    return;
  }
    ...
    return Scaffold(
      body: Container(
        decoration: const BoxDecoration(
          color: Color(0xFFF8FAFC), // Modern off-white background
        ),
        child: SafeArea(
          child: Stack(
            children: [
...
''';
  ...
  content = content.substring(0, buildIndex) + newBuild;
